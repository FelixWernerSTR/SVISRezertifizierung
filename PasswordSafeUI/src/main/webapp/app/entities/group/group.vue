<template>
  <div>
    <h2 id="page-heading" data-cy="GroupHeading">
      <span v-text="$t('passwordSafeUiApp.group.home.title')" id="group-heading">Groups</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('passwordSafeUiApp.group.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'GroupCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('passwordSafeUiApp.group.home.createLabel')"> Create a new Group </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && groups && groups.length === 0">
      <span v-text="$t('passwordSafeUiApp.group.home.notFound')">No groups found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('passwordSafeUiApp.group.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('passwordSafeUiApp.group.description')">Description</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="passwordSafeUiApp.group.delete.question" data-cy="groupDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-group-heading" v-text="$t('passwordSafeUiApp.group.delete.question', { id: removeId })">
