...
public interface EntryRepository extends JpaRepository<Entry, Long> {
    ...
    @Query(
        value = "select distinct entry from Entry entry left join fetch entry.group",
        countQuery = "select count(distinct entry) from Entry entry"
    )
    Page<Entry> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct entry from Entry entry left join fetch entry.group")
    List<Entry> findAllWithToOneRelationships();

    @Query("select entry from Entry entry left join fetch entry.group where entry.id =:id")
