<template>
  <div>
    <h2 id="page-heading" data-cy="BeitragHeading">
      <span v-text="$t('svisAktuellesApp.beitrag.home.title')" id="beitrag-heading">Beitrags</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('svisAktuellesApp.beitrag.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'BeitragCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('svisAktuellesApp.beitrag.home.createLabel')"> Create a new Beitrag </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && beitrags && beitrags.length === 0">
      <span v-text="$t('svisAktuellesApp.beitrag.home.notFound')">No beitrags found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="beitrags">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('content')">
              <span v-text="$t('svisAktuellesApp.beitrag.content')">Content</span>
            ...
            <th scope="row" v-on:click="changeOrder('attrib')">
              <span v-text="$t('svisAktuellesApp.beitrag.attrib')">Attrib</span>
            ...
            <th scope="row" v-on:click="changeOrder('titel')">
              <span v-text="$t('svisAktuellesApp.beitrag.titel')">Titel</span>
            ...
            <th scope="row" v-on:click="changeOrder('rechte')">
              <span v-text="$t('svisAktuellesApp.beitrag.rechte')">Rechte</span>
            ...
            <th scope="row" v-on:click="changeOrder('validFrom')">
              <span v-text="$t('svisAktuellesApp.beitrag.validFrom')">Valid From</span>
            ...
            <th scope="row" v-on:click="changeOrder('validTo')">
              <span v-text="$t('svisAktuellesApp.beitrag.validTo')">Valid To</span>
            ...
            <th scope="row" v-on:click="changeOrder('publishDate')">
              <span v-text="$t('svisAktuellesApp.beitrag.publishDate')">Publish Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('archiv')">
              <span v-text="$t('svisAktuellesApp.beitrag.archiv')">Archiv</span>
            ...
            <th scope="row" v-on:click="changeOrder('thema.name')">
              <span v-text="$t('svisAktuellesApp.beitrag.thema')">Thema</span>
        ...
        <tbody>
          <tr v-for="beitrag in beitrags" :key="beitrag.id" data-cy="entityTable">
            ...
            <td v-text="$t('svisAktuellesApp.Archiv.' + beitrag.archiv)">{{ beitrag.archiv }}</td>
            <td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="svisAktuellesApp.beitrag.delete.question" data-cy="beitragDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-beitrag-heading" v-text="$t('svisAktuellesApp.beitrag.delete.question', { id: removeId })">
