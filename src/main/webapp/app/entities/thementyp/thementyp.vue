<template>
  <div>
    <h2 id="page-heading" data-cy="ThementypHeading">
      <span v-text="$t('svisInfoApp.thementyp.home.title')" id="thementyp-heading">Thementyps</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('svisInfoApp.thementyp.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ThementypCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('svisInfoApp.thementyp.home.createLabel')"> Create a new Thementyp </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && thementyps && thementyps.length === 0">
      <span v-text="$t('svisInfoApp.thementyp.home.notFound')">No thementyps found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="thementyps">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('svisInfoApp.thementyp.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="svisInfoApp.thementyp.delete.question" data-cy="thementypDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-thementyp-heading" v-text="$t('svisInfoApp.thementyp.delete.question', { id: removeId })">
