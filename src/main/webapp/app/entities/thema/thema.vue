<template>
  <div>
    <h2 id="page-heading" data-cy="ThemaHeading">
      <span v-text="$t('svisInfoApp.thema.home.title')" id="thema-heading">Themas</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('svisInfoApp.thema.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ThemaCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('svisInfoApp.thema.home.createLabel')"> Create a new Thema </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && themas && themas.length === 0">
      <span v-text="$t('svisInfoApp.thema.home.notFound')">No themas found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('svisInfoApp.thema.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('rechte')">
              <span v-text="$t('svisInfoApp.thema.rechte')">Rechte</span>
            ...
            <th scope="row" v-on:click="changeOrder('displaycount')">
              <span v-text="$t('svisInfoApp.thema.displaycount')">Displaycount</span>
            ...
            <th scope="row" v-on:click="changeOrder('thementyp.name')">
              <span v-text="$t('svisInfoApp.thema.thementyp')">Thementyp</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="svisInfoApp.thema.delete.question" data-cy="themaDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-thema-heading" v-text="$t('svisInfoApp.thema.delete.question', { id: removeId })">
