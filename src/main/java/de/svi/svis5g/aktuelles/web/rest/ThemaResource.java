...
/**
 * REST controller for managing {@link de.svi.svis5g.aktuelles.domain.Thema}.
 */
@RestController
@RequestMapping("/api")
public class ThemaResource {
    ...
    public ResponseEntity<Thema> createThema(@Valid @RequestBody Thema thema) throws URISyntaxException {
        ...
        Thema result = themaService.save(thema);
    ...
    public ResponseEntity<Thema> updateThema(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Thema thema)
        ...
        Thema result = themaService.update(thema);
        return ResponseEntity
            .ok()
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/themas/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Thema> partialUpdateThema(
        ...
        Optional<Thema> result = themaService.partialUpdate(thema);
    ...
    public ResponseEntity<List<Thema>> getAllThemas(
        ...
        Page<Thema> page;
        if (eagerload) {
            page = themaService.findAllWithEagerRelationships(pageable);
        } else {
            page = themaService.findAll(pageable);
        }
    ...
    public ResponseEntity<Thema> getThema(@PathVariable Long id) {
        ...
        Optional<Thema> thema = themaService.findOne(id);
